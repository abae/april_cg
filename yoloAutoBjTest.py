...
def getHandInArea(x, y):
    ...
    width = 640
    height = 640
    # im = im.resize((375, 150), Image.LANCZOS)
    image_pil = Image.new("RGB", (width, height), "white")
    image_pil.paste(im, (100, 120))

    image_pil.save("tmp/.tmpBJcapture.png")
    ...
    if debug:
        for result in results:
            ...
            result.show()
    hand = []
    cpu_result = results[0].cpu()
    for card in cpu_result.boxes.data:
        hand.append(getValue(yaml_data.get(int(card.numpy()[5]))[:-1]))
    print(hand)
    return hand
