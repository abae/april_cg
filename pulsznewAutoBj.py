...
def get_image_pos_on_screen(image_path):  
    try:
        global data
        location = pyautogui.locateOnScreen(image_path, confidence=0.995, region=(data['none']['x']+data['check_area']['x'], data['none']['y']+data['check_area']['y'], data['check_area']['w'], data['check_area']['h']))
        if location:
...
def getHand(xHand, yHand):
    ...
    for rank_contour in rank_contours:
        ...
        image_pil = image_pil.point(lambda p: 255 if p > 128 else 0)
        image_pil = image_pil.resize((int(width*scaling), int(height*scaling)), Image.Resampling.LANCZOS)
        ...
        if text:
            ...
            print(f"failed to find see contour {sys.argv[1]} (assuming it's 8)")
            texts += '8'
...
def playGame(splitStatus, dealerHand):
    ...
    if playerHand[1] >=21 or playerHand[1] == -1:
        return "end"
    #Find out the play
    ...
    if playerAction == "hit":
        ...
        num_split += 1
        loop += 1
        # if not data['ace_split_play'] and playerHand[1] == 12:
        #     if data['resplit_ace']:
        #         doAction('split')
        #     return "end"
        if splitStatus == 'none':
